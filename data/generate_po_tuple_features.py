...
def main(mention_file, sense_mapping, save_file, use_all_svos, pca_dim):
    ...
    ## Added by CHF to map each vocaboray to sentence id 
    sent_id_2_vocab = defaultdict(list)
    ## Ended Add
    ...
    for svo_id, svo_info in tqdm(po_mentions.items()):
        ...
        sent_id, _ = svo_id.split("_")
        sent_id_2_vocab[sent_id].append(vs_oh_tuple)
    ...
    sent_id_2_vocab_freq = defaultdict(list)
    for sent_id, vocab_list in sent_id_2_vocab.items():
        for vv in vocab_list:            
            sent_id_2_vocab_freq[sent_id].append(vv+(vs_w_oh2freq[vv],)+(vocab[vv],))
    ...
    with open(save_file, "wb") as f:
        pk.dump({
            ...
            "sent_id_2_vocab_freq": sent_id_2_vocab_freq         
